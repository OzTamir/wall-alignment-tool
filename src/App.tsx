...
function App() {
  ...
  const calculateNailPositions = useCallback((measurements: Measurements): number[] => {
        ...
        <div className="bg-white p-6 rounded-lg shadow-md">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700">Wall Length (cm)</label>
                <input
                  type="number"
                  value={wallLength}
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700">Image Width (cm)</label>
                <input
                  type="number"
                  value={imageWidth}
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700">Image Height (cm)</label>
                <input
                  type="number"
                  value={imageHeight}
                ...
                <div className="space-y-2">
                  {nailPositions.map((position, index) => (
                    <p key={index} className="text-red-600">
                      Nail #{index + 1}: {position.toFixed(1)} cm from wall start
                    </p>
                  ))}
                </div>
              ) : (
          ...
          <div className="relative h-40 bg-gray-100 rounded-lg overflow-hidden">
            {isValid && (
              <div className="absolute inset-0 flex items-center">
                {/* Left wall space */}
                <div 
                  style={{ width: `${(measurements.wallToFirst / wallLength) * 100}%` }}
                ...
                {Array.from({ length: imageCount }).map((_, index) => (
                  <React.Fragment key={index}>
                    ...
                    {index < imageCount - 1 && (
                      <div 
                        style={{ width: `${(measurements.betweenImages / wallLength) * 100}%` }}
                ...
                {/* Right wall space */}
                <div 
                  style={{ width: `${(measurements.lastToWall / wallLength) * 100}%` }}
